...
export const GET = async (req: NextRequest) => {
  ...
  const startDateBanParam = searchParams.get("startDateBan");
  const endDateBanParam = searchParams.get("endDateBan");
  ...
  try {
    ...
    const startDateBan = startDateBanParam ? new Date(startDateBanParam) : null;
    const endDateBan = endDateBanParam ? new Date(endDateBanParam) : null;
    ...
    if (startDateBan && endDateBan) {
      (query as any).updatedAt = { $gte: startDateBan, $lte: endDateBan };
    } else if (startDateBan) {
      (query as any).updatedAt = { $gte: startDateBan };
    } else if (endDateBan) {
      (query as any).updatedAt = { $lte: endDateBan };
    }
