...
export async function GET(
  ...
  try {
    ...
    const post = await Post.findOne({
      _id: params.id,
      ...(isAdmin ? {} : { creator: session.user.id }),
    });
    ...
    const comments = await Comment.find({ post: params.id }).select("content");
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: `You are an assistant for Le-Gallerie, a social media app focused on photography and visual arts. Summarize the following user comments in a friendly and concise way:\n\n${commentListToSummarize}. And keep it brief and just return the summary and nothing else`,
            },
          ],
        },
      ],
