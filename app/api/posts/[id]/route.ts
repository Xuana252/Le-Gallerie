...
export const GET  = async (req: NextApiRequest, {params}:{params: {id:string}}) => {
    try {
        await connectToDB();
        const post = await Post.findById(params.id).populate('creator').populate('categories');
        ...
        return NextResponse.json({message: 'failed to fetch for post'},{status:500})
    }
}
...
export const PATCH = async (req: NextRequest, {params}:{params: {id:string}}) => {
    ...
    try {
        ...
        const post = await Post.findByIdAndUpdate(params.id,updatedPost,{new:true})
        if(post) {
