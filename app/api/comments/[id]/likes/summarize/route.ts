...
export const GET = async (
  ...
  try {
    ...
    const reactionCounts = await CommentLike.aggregate([
      { $match: { comment: new ObjectId(params.id) } },
      {
        $group: {
          _id: "$reaction",
          count: { $sum: 1 },
    ...
    console.error("Failed to summarize comment likes", error);
    return NextResponse.json(
      { message: "Failed to summarize comment likes" },
      { status: 500 }
    );
  }
};
