...
import { Like, User, type Post } from "@lib/types";
      ...
      {likes.map((like,index) =>
        <label key={index} className="flex items-center gap-2">
          {session?.user?.id === like.user._id ? (
                    ...
                    <h2 className="text-sm cursor-pointer text-accent">{like.user.username}</h2>
        </label>
      )}
    </ul>
  </div>
              ...
              <div className="flex-row p-2 h-fit z-10 flex sticky bg-secondary-1 top-[60px] gap-2">
                {postRendered && (
                  <div className="flex justify-start items-center">
                    ...
                    <DropDownButton hover={true} dropDownList={likedUser} dropDirection={'right'} >
              ...
              <div className="grow flex-col flex gap-2">
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm cursor-pointer">{post?.creator.username}</h2>
                  </label>
                </div>
