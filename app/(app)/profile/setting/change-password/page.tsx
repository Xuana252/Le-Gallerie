...
import { changeUserPassword, sendVerificationCode } from "@server/accountActions";
...
export default function ChangePassword() {
  ...
  const handleSendVerification = async () => {
    ...
    try {
      const response = await sendVerificationCode(session?.user.id);
      setVerificationCode(response.toString().trim());
    } catch (error) {
  ...
  useEffect(() => {
    if (count <= 0) {
      ...
      setIsTimeout(false);
      setCount(10); // Optional: disable timeout when countdown ends
    }
  }, [count]);
  return (
